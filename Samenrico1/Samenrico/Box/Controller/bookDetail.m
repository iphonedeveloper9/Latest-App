...
- (void)viewDidLoad
{
    ...
    UITapGestureRecognizer *fulltapGesture = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(full_issueClick:)];
    fulltapGesture.delegate = (id <UIGestureRecognizerDelegate>)self;
    [self.fuull_issue addGestureRecognizer:fulltapGesture];
    
    //[self performSelector:@selector(acctionGetBookDetail) withObject:nil afterDelay:0.01];
    //[self performSelector:@selector(GetAllCategoryData) withObject:nil afterDelay:0.01];
    [self setframe];
...
-(void)setframe
{
    ...
    self.tblData.frame = CGRectMake(self.tblData.frame.origin.x, self.tblData.frame.origin.y - appDelegate.posY, self.tblData.frame.size.width, self.tblData.frame.size.height + appDelegate.posY);
    self.btn_BackButton.frame = CGRectMake(self.btn_BackButton.frame.origin.x, self.btn_BackButton.frame.origin.y - appDelegate.posY, self.btn_BackButton.frame.size.width, self.btn_BackButton.frame.size.height);
}
...
- (void)full_issueClick:(UITapGestureRecognizer *)tapGesture
{
    NSLog(@"enter the full_issueClick");
    Full_IssueViewController *full = [[Full_IssueViewController alloc] initWithNibName:@"Full_IssueViewController" bundle:nil];
    full.arrArticale = self.arrayArticle;
...
-(void)GetAllCategoryData
{
    ...
    NSString *JSON = @"";
    JSON = [JSON stringByAppendingFormat:@"{\"method\":\"item_detail\",\"params\":{\"id\":\"%@\",\"publish_type\":\"%@\"}}",self.strBookID,self.strBookType];
    
    //NSString *theJSON = @"";
    //theJSON = [theJSON stringByAppendingFormat:@"{\"method\":\"item_detail\",\"params\":{\"id\":\"%@\",\"publish_type\":\"%@\"}}",@"6",@"magazine"];
    
    NSString *url = [NSString stringWithFormat:@"http://www.samenrico.com/api_new/json_check.php?requestJson=%@", JSON];
    
    NSLog(@"%@",url);
    
    url = [url stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
    NSURL *urlCategory = [NSURL URLWithString:url];
    NSMutableURLRequest *categoryRequest = [NSMutableURLRequest requestWithURL:urlCategory cachePolicy:NSURLRequestUseProtocolCachePolicy timeoutInterval:120];
    
    getAllarticalesCon = [NSURLConnection connectionWithRequest:categoryRequest delegate:self];
...
- (void)connection:(NSURLConnection *)connection didReceiveResponse:(NSURLResponse *)response {
    ...
    if(connection == getAllarticalesCon)
    {
...
- (void)connection:(NSURLConnection *)connection didFailWithError:(NSError *)error {
    ...
    if(connection == getAllarticalesCon)
    {
        ...
        UIAlertView *alert=[[UIAlertView alloc] initWithTitle:@"Alert" message:@"Connection did fail!" delegate:self cancelButtonTitle:@"OK" otherButtonTitles:nil, nil];
        [alert show];
    }
}
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection
{
    ...
    {
        NSError *myError = nil;
        NSMutableDictionary *dictAllCategories = [[NSMutableDictionary alloc]init];
        dictAllCategories = [NSJSONSerialization JSONObjectWithData:fetchData options:NSJSONReadingMutableLeaves error:&myError];
        
        NSLog(@"%@", dictAllCategories);

        self.arrayArticle = [[dictAllCategories objectForKey:@"info"] objectForKey:@"article_list"];
        ...
        //NSLog(@"%@",self.arrayArticle);
        //NSLog(@"%@",self.FreeArticle);
...
- (void)dealloc {
    ...
    [_tblData release];
    [_about_scrb release];
    [_fuull_issue release];
    [_header_view release];
    [_btn_BackButton release];
    [super dealloc];
}
/////
...
- (void)viewDidUnload {
    ...
    [self setFuull_issue:nil];
    [self setHeader_view:nil];
    [self setBtn_BackButton:nil];
    [super viewDidUnload];
}
@end
